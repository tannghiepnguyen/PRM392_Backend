...
namespace PRM392_Backend.Service.Service
{
    public class ServiceManager : IServiceManager
    {
        ...
        public ServiceManager(IRepositoryManager repositoryManager, UserManager<User> userManager, IConfiguration configuration, IMapper mapper, IBlobService blobService, IHttpContextAccessor httpContextAccessor)
        {
            ...
            cartService = new Lazy<ICartService>(() => new CartService(repositoryManager, mapper, httpContextAccessor));
            cartItemService = new Lazy<ICartItemService>(() => new CartItemService(repositoryManager, mapper, httpContextAccessor));
            orderService = new Lazy<IOrderService>(() => new OrderService(repositoryManager, mapper, httpContextAccessor));
            chatMessageService = new Lazy<IChatMessageService>(() => new ChatMessageService(repositoryManager, mapper, httpContextAccessor));
            paymentService = new Lazy<IPaymentService>(() => new PaymentService(repositoryManager, mapper));
